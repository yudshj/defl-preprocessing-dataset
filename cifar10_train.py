...
with tf.device('/cpu:0'):
    ds = load_data_x_y('cifar10_x_train.npy', 'cifar10_y_train.npy', False, True, True).map(lambda img,label: (data_augmentation(img),label), num_parallel_calls=8)
...
model = tf.keras.models.load_model('./cifar10_init_model.h5')
model.summary()
...
history = model.fit(
    ...
    validation_freq=1,
    callbacks=[tf.keras.callbacks.CSVLogger('cifar10_train.log')]
)
